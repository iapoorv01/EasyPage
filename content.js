...
function replaceSelectionWithSimplified(range, simplifiedText) {
    ...
    wrapper.title = 'This text was simplified by EasyPage';
...
async function activateJargonDecoder() {
    ...
    if (!apiConfig.hasKey) {
        console.warn('EasyPage: No API key configured');
        hideProgressLoader();
        state.isGenerating = false;
        showNotification('Please configure an API key (OpenRouter or Gemini)', 'error');
        return;
    ...
    try {
        ...
        console.error('EasyPage: Jargon decoder error:', error);
        hideProgressLoader();
        state.isGenerating = false;
        state.abortController = null;
...
console.log('EasyPage content script loaded');
